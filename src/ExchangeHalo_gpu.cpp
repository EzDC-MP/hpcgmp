...
#if !defined(HPCG_NO_MPI) & (defined(HPCG_WITH_CUDA) | defined(HPCG_WITH_HIP) | defined(HPCG_WITH_KOKKOSKERNELS))
#include <mpi.h>
...
void ExchangeHalo_ref(const SparseMatrix_type & A, Vector_type & x) {
  ...
  //
  TICK();
#if defined(HPCG_WITH_HIP) // Only with HIP for now
  ...
  for (int i = 0; i < num_neighbors; i++) {
    ...
    #if defined(HPCG_USE_GPU_AWARE_MPI) & defined(HPCG_WITH_HIP)
